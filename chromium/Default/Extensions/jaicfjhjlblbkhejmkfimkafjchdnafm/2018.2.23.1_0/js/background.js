...
chrome.runtime.onInstalled.addListener(function(details) {
	...
	if (details.reason == "install") {
		...
		chrome.storage.sync.get(['PinnedList', 'PinnedListURLs', 'PinnedListLW', 'PinnedListImg', 'lastVideo'], function(items) {
			...
			chrome.storage.sync.remove('enableHTML5Fix');
			chrome.storage.sync.remove('enableHomepageAds');
			chrome.storage.sync.remove('enableLatestEpisode');
			chrome.storage.sync.remove('enablePinnedTooltips');

			/* Potentially new storage format
				var pinnedList = [
					{
						title: '',
						url: '',
	...
	callback: {
		block: function(details) {
			if (activeTabs.has(details.tabId)) {
				...
				// if (/kissanime\.ru|kissmanga\.com|kissasian\.ch|kimcartoon\.me|readcomiconline\.to/.test(tabURL.hostname)) {
...
chrome.webRequest.onBeforeRequest.addListener(function(details) {
	...
	if (details.type === 'main_frame' && /\/Anime\/.+\/.+&s=(beta|default)/i.test(tabURL.href)) {
		return { redirectUrl: tabURL.href.replace(/&pfail=([^&]+)/gi, '').replace(/&s=([^&]+)/i, '&s=beta&pfail=1') }
	}
